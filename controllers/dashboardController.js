...
exports.getAdminDashboardStats = catchAsync(async (req, res, next) => {
  try {
    ...
    const activeReaderIds = await Loan.distinct("readerId", { status: "active" });

    // Parallel queries for better performance
    const [totalBooks, activeReaders, activeLoans, totalOutstandingFines, booksStats, loanTrends, overdueLoans] =
      await Promise.all([
        // Total books count
        Book.countDocuments(),
    ...
    const [currentMonthBooks, lastMonthBooks, currentMonthReaders, lastMonthReaders] = await Promise.all([
      Book.countDocuments({ createdAt: { $gte: currentMonthStart } }),
      Book.countDocuments({ createdAt: { $gte: lastMonth, $lt: currentMonthStart } }),
      Reader.countDocuments({ createdAt: { $gte: currentMonthStart } }),
      Reader.countDocuments({ createdAt: { $gte: lastMonth, $lt: currentMonthStart } }),
    ]);

    // Calculate percentage changes
    const bookGrowth =
      lastMonthBooks > 0 ? (((currentMonthBooks - lastMonthBooks) / lastMonthBooks) * 100).toFixed(1) : 0;
    const readerGrowth =
      lastMonthReaders > 0 ? (((currentMonthReaders - lastMonthReaders) / lastMonthReaders) * 100).toFixed(1) : 0;
...
exports.getReaderDashboardStats = catchAsync(async (req, res, next) => {
  ...
  try {
    const [activeLoans, dueSoonLoans, outstandingFines, totalBooksRead, readerHistory] = await Promise.all([
    ...
    return next(new AppError("Error fetching reader dashboard statistics", 500));
  }
});
